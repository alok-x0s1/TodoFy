...
import { generateHashedPassword, verifyPassword } from "../utils/hashPassword.js";
...
const signupUser = async (req, res) => {
    try {
        ...
        if(existedUser) {
            return res.status(400).send("User with email or password already exist.")
        }
        ...
        if(!createdUser) {
            return res.status(500).send("Internal server error :: Error while creating the user")
        }
